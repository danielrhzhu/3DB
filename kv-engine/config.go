...
func random_handles(kvh []*networking.ClientEnd) []*networking.ClientEnd {
	sa := make([]*networking.ClientEnd, len(kvh))
	copy(sa, kvh)
	for i := range sa {
		j := rand.Intn(i + 1)
		sa[i], sa[j] = sa[j], sa[i]
...
func (cfg *config) makeClient(to []int) *Clerk {
	...
	ends := make([]*networking.ClientEnd, cfg.n)
...
func (cfg *config) StartServer(i int) {
	...
	kvsvc := networking.MakeService(cfg.kvservers[i])
	rfsvc := networking.MakeService(cfg.kvservers[i].rf)
	srv := networking.MakeServer()
...
func make_config(t *testing.T, n int, unreliable bool, maxraftstate int) *config {
	...
	cfg := &config{}
	cfg.t = t
	cfg.net = networking.MakeNetwork()
	cfg.n = n
